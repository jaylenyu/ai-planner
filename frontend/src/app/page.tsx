...
import { ArrowRight, CalendarCheck, Compass, MapIcon, MessageCircle, Sparkles, Timer } from 'lucide-react';
...
export default function Home() {
  return (
    <div className="min-h-screen bg-white" style={{ color: 'var(--text-primary)' }}>
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md"
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
          ...
          {/* Desktop nav */}
          <nav className="hidden items-center gap-6 text-sm md:flex" aria-label="메인 메뉴">
            <Link href="#features" className="transition-colors hover:text-[var(--text-primary)]"
                  style={{ color: 'var(--text-secondary)' }}>기능 소개</Link>
            <Link href="#workflow" className="transition-colors hover:text-[var(--text-primary)]"
                  style={{ color: 'var(--text-secondary)' }}>작동 방식</Link>
            <Link href="#stories" className="transition-colors hover:text-[var(--text-primary)]"
                  style={{ color: 'var(--text-secondary)' }}>후기</Link>
          ...
          {/* Mobile CTA */}
          <PrimaryButton asChild variant="brand" size="sm" className="md:hidden">
      ...
      <main>
        {/* ── HERO ── */}
        <section className="relative overflow-hidden hero-pattern" aria-labelledby="hero-heading">
          <div className="mx-auto flex max-w-6xl flex-col items-center gap-12 px-4 sm:px-6 py-20 lg:flex-row lg:gap-16 lg:py-28">
            ...
            <div className="flex-1 space-y-8 animate-fade-in-up">
              ...
              <h1
                ...
                style={{ fontSize: 'var(--font-size-display)', color: 'var(--text-primary)' }}
              >
                대화 한 번으로<br />
              ...
              <ul className="flex flex-wrap gap-5 text-sm" style={{ color: 'var(--text-tertiary)' }}>
                <li className="flex items-center gap-1.5">
                  <Sparkles className="h-3.5 w-3.5 text-orange-400" aria-hidden="true" />
                  맞춤 취향 분석
                </li>
                <li className="flex items-center gap-1.5">
                  <Timer className="h-3.5 w-3.5 text-orange-400" aria-hidden="true" />
                  동선 시간 계산
                </li>
                <li className="flex items-center gap-1.5">
                  <CalendarCheck className="h-3.5 w-3.5 text-orange-400" aria-hidden="true" />
                  캘린더 연동
                </li>
              </ul>
            </div>
            ...
            {/* Preview card */}
            <div className="w-full max-w-sm flex-shrink-0 animate-fade-in-up lg:w-96"
                 style={{ animationDelay: '120ms' }}>
              <AppCard padding="lg" className="shadow-[var(--shadow-card-hover)]">
                <div className="mb-5">
                ...
                <ol className="flex flex-col gap-3">
                  {previewItems.map((item, i) => (
                    <li key={i} className="flex items-center gap-3">
                      ...
                      <div className="flex flex-1 items-center justify-between rounded-xl px-3 py-2"
                           style={{ background: 'var(--surface-sunken)' }}>
                        <span className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>
                          {item.name}
                        </span>
                        <span className="ml-2 flex-shrink-0 text-xs" style={{ color: 'var(--text-tertiary)' }}>
                          {item.time}
                        </span>
                      </div>
                    </li>
        ...
        {/* ── STATS ── */}
        <SectionLayout background="alt" eyebrow="By the numbers" heading="숫자로 보는 Dayplan">
          <div className="grid gap-5 sm:grid-cols-3">
            {stats.map((stat, i) => (
              <AppCard key={stat.label} padding="lg" style={{ animationDelay: `${i * 80}ms` }}>
                <p className="mb-3 text-sm font-medium" style={{ color: 'var(--text-tertiary)' }}>
                  {stat.label}
                </p>
        ...
        >
          <div className="grid gap-5 md:grid-cols-3 stagger-children">
            {features.map((f) => (
              <AppCard key={f.title} padding="lg" hoverable className="flex flex-col gap-5">
                <div className={`inline-flex h-11 w-11 items-center justify-center rounded-2xl ${f.bg}`}
                     aria-hidden="true">
                  <f.icon className={`h-5 w-5 ${f.color}`} />
                </div>
                <div>
                  <h3 className="mb-1.5 font-bold" style={{ color: 'var(--text-primary)' }}>
                    {f.title}
                  </h3>
        ...
        <SectionLayout
          ...
          eyebrow="Voices"
          heading="사용자 후기"
          subheading="Dayplan을 먼저 경험한 사람들의 생생한 평가입니다."
        >
          <div className="grid gap-5 md:grid-cols-2 stagger-children">
            {testimonials.map((t) => (
              <AppCard key={t.name} padding="lg" className="flex flex-col gap-5">
                <svg className="h-7 w-7 text-orange-200" fill="currentColor" viewBox="0 0 24 24"
                     aria-hidden="true">
                  <path d="M4.583 17.321C3.553 16.227 3 15 3 13.011c0-3.5 2.457-6.637 6.03-8.188l.893 1.378c-3.335 1.804-3.987 4.145-4.247 5.621.537-.278 1.24-.375 1.929-.311 1.804.167 3.226 1.648 3.226 3.489a3.5 3.5 0 01-3.5 3.5 3.871 3.871 0 01-2.748-1.179zm10 0C13.553 16.227 13 15 13 13.011c0-3.5 2.457-6.637 6.03-8.188l.893 1.378c-3.335 1.804-3.987 4.145-4.247 5.621.537-.278 1.24-.375 1.929-.311 1.804.167 3.226 1.648 3.226 3.489a3.5 3.5 0 01-3.5 3.5 3.871 3.871 0 01-2.748-1.179z" />
                </svg>
                <blockquote className="flex-1 text-base font-medium leading-relaxed"
        ...
        {/* ── CTA ── */}
        <section className="py-[var(--space-section)]" aria-labelledby="cta-heading">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            ...
            >
              <p className="label-eyebrow mb-4" style={{ color: 'rgba(255,255,255,0.7)' }}>
                Ready?
              </p>
              <h2
                id="cta-heading"
