...
export async function POST() {
  ...
  if (!email || !password) {
    return NextResponse.json({ message: 'publicadmin 계정이 구성되지 않았습니다.' }, { status: 503 });
  }
  ...
  if (!payload) {
    return NextResponse.json({ message: '유효한 관리자 토큰을 발급하지 못했습니다.' }, { status: 401 });
  }
  ...
  nextResponse.cookies.set(ADMIN_ACCESS_COOKIE, data.access_token, getAdminCookieOptions(ADMIN_ACCESS_COOKIE_MAX_AGE));
  nextResponse.cookies.set(ADMIN_REFRESH_COOKIE, data.refresh_token, getAdminCookieOptions(ADMIN_REFRESH_COOKIE_MAX_AGE));
  return nextResponse;
}
